...
void draw_vl_up(s16 x, s16 y, s16 l, u32 color)
{
	register int i;
	for (i=0; i<= l; i++)
		set_pixel(x, y-i, color);
...
void gradient_fill(u32 x1, u32 x2, u32 y1, u32 y2)
{
	...
	u16 strip = (y2-y1)/30;
	u16 j=0;
	u32 color_code[]= {0x5fc8e6, 0x53b8d9, 0x48a9cb, 0x3d99be, 0x328ab0, 0x2e81a7, 0x2a799e, 0x267095, 0x276b8f, 0x286688, 0x286282, 0x285d7c};
...
/*
 * @desc : print_int : prints integer
 *
 * 	@arg length : keep it 0
 * 	@arg filler : keep it 0
 *
 *
 */
...
{
	HAL_Delay(200);
	SSD1963_CTRLPORT->BSRR = SSD1963_PIN_RST << 16;
	HAL_Delay(100);
