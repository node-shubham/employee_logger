...
//  MSS_SPI_set_slave_select( &g_mss_MFRC522_SPI, MSS_SPI_SLAVE_0 );
  ...
  //   them as is
//  rx_bits = MSS_SPI_transfer_frame( &g_mss_MFRC522_SPI, (((addr << 1) & 0x7E) << 8) |  val );
  //HAL_SPI_TransmitReceive(&MFRC522_SPI, (((addr << 1) & 0x7E) << 8) |  val , rx_bits, 1, 500);
  HAL_SPI_Transmit(&MFRC522_SPI, &addr_bits, 1, 500);
  HAL_SPI_Transmit(&MFRC522_SPI, &val, 1, 500);
  // clear the select line-- we are done here
//  MSS_SPI_clear_slave_select( &g_mss_MFRC522_SPI, MSS_SPI_SLAVE_0 );
...
//  rx_bits = MSS_SPI_transfer_frame( &g_mss_MFRC522_SPI, ((((addr << 1) & 0x7E) | 0x80) << 8) | 0x00 );
  //HAL_SPI_TransmitReceive(&MFRC522_SPI, ((((addr << 1) & 0x7E) | 0x80) << 8) | 0x00 , rx_bits, 1, 500);
//HAL_SPI_Transmit(&MFRC522_SPI, (unsigned char*) ((((addr<<1) & 0x7E) | 0x80)), 1, 500);
HAL_SPI_Transmit(&MFRC522_SPI, &addr_bits, 1, 500);

  HAL_SPI_Receive(&MFRC522_SPI, &rx_bits, 1, 500);
  // clear the select line-- we are done here
//  MSS_SPI_clear_slave_select( &g_mss_MFRC522_SPI, MSS_SPI_SLAVE_0 );
